...
- **Phase 2 — backend scaffold delivered, frontend swap pending.** FastAPI skeleton, Alembic + PostgreSQL (plain, no TimescaleDB — decision #2), Event Bus with two dispatch lanes (decision #9, verified: a test proves a slow normal-lane subscriber cannot delay a critical-lane event) + Market Clock, `DebounceScheduler` utility (decision #10, unit-tested but not yet wired to a real consumer — Market State Engine doesn't exist until Phase 5), WebSocket gateway. Round-trip exit criterion (dummy event through the Event Bus, out over WebSocket) verified live end-to-end — see `backend/README.md`'s "Verifying the Phase 2 round-trip" section. Remaining for Phase 2: swap frontend mock data for the same shapes, pushed live.
