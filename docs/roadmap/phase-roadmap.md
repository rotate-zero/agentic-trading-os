...
- **Phase 3 — broker adapter built, live connection unverified.** Broker order revised: IBKR only (decision #1) — Alpaca isn't open to Bangladesh-resident accounts. `IBKRAdapter` implements the full `BrokerAdapter` interface via `ib_async` (decision #13); `IBKRIngestBridge` buckets ticks into 1-minute candles as a Phase-3-minimal stand-in for Phase 4's real Market Data Engine (decision #16). Verified: `ib_async` API signatures via introspection, tick-bucketing logic via unit tests, and the connect path via a real (refused) socket connection attempt. **Not verified: an actual live connection to a running Gateway with a real IBKR account** — no path to that in this environment. 2FA strategy documented (decision #14): IB Key + Gateway auto-restart, not SMS, not a claim of full headless operation.
