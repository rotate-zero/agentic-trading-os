...
**Status:** CONFIRMED — `vwap_ext`/`session_volume_ext` (decision #100), `premarket_volume_ratio` (decision #101), Scanner integration (decision #102). All three built and verified across three rounds (§6/§7/§8 below record each in full); backfilled into the decision log together after decision #99 found this doc's status line had gone stale against real, merged code.
**Owner:** Saqib
