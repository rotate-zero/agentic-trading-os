...
**Status:** Stage 0 confirmed (`confirmed-decisions.md` #50). **Stages 1 and 3 are both functionally complete.** Stage 1: the Chart consumes Feature Engine for SMA/EMA/VWAP as continuous overlay lines (#54) and PDH/PDL/PDC/Camarilla/pre-market H/L/VPOC as horizontal price lines (#58), across `1m`/`5m`/`15m`/`1h` throughout. Stage 2's backend piece confirmed and built (#51), its frontend half (2.3/2.4) closed out as a side effect of Stage 1. Stage 3: every indicator on the original roadmap — EMA (#52), VWAP (#53), PDH/PDL/PDC + Camarilla + pre-market H/L (#56), and VPOC (#57, resolving D5 more simply than anticipated: the frontend's own VPOC is previous-day-scoped, not a live-growing histogram) — is now computed backend-side. **Executed out of the original sequence throughout** — Stage 2 and all of Stage 3 landed before Stage 1 did, since their design questions were more time-sensitive to lock in than the order this doc originally listed; Stage 1 then covered every built indicator at once each time, rather than one at a time, once it was clear the marginal cost of doing so was small. **Remaining:** Stage 4 (retiring the frontend indicator files) — **investigated (temp id `chart-migration-stage-4-flagging`), not actionable.** Checklist below: 0 of the 7 candidate files were found dead. Each still has a real, currently-exercised local-fallback caller directly inside `utils/indicators.ts`'s own dispatcher — arbitrary/non-configured SMA/EMA periods, any timeframe outside `1m`/`5m`/`15m`/`1h`, and (for the four horizontal-level files) the cold-start absence of a previous trading day within the configured lookback. This is exactly what decisions #54/#58 already stated in so many words when Stage 1 built this fallback path ("NOT retired... still the fallback path, still real code something calls") — Stage 3 completing backend coverage narrows *when* the fallback fires, it doesn't remove the two structural reasons it still can. See the trigger condition recorded in `future-ideas.md` #26 for what would actually make this stage's files flaggable. Also still remaining: a real-browser verification pass across all of Stages 1 and 3, still not possible in this environment.
**Owner:** Saqib
...
**Executed (temp id `chart-migration-stage-4-flagging`) — result: nothing to flag. Details below per sub-item; full reasoning in the matching `confirmed-decisions.md` entry.**

- [x] **4.1** — For each of `sma.ts`, `ema.ts`, `vwap.ts`, `previousDayLevels.ts`, `premarketLevels.ts`, `camarillaPivots.ts`, `vpoc.ts`: confirmed via `utils/indicators.ts`'s dispatcher *and* a repo-wide grep for direct imports (not just the dispatcher) that `utils/indicators.ts` is the only importer of any of them — but in **every one of the seven cases**, the imported function is directly invoked, not dead-imported: `computePriceIndicator`'s `SMA`/`EMA`/`VWAP` branches call `sma()`/`ema()`/`vwap()` whenever `backendSeries` has no entry for the exact `(period, timeframe)` an instance is configured for (the chart still allows arbitrary SMA/EMA periods 2–500; Feature Engine only computes the configured `[9, 20, 50]`/`[9, 20]` defaults for `1m`/`5m`/`15m`/`1h`), and `resolveHorizontalLevelPrice()` calls `computePreviousDayLevels()`/`computePremarketLevels()`/`computeCamarillaPivots()`/`computeVPOC()` whenever `backendLevels` has no entry for that level (no previous trading day within the configured lookback yet — an honest cold-start gap, not a coverage gap Stage 3 closes by existing). **0 of 7 excluded selectively — all 7 have a real, currently-exercised caller**, so none is force-fit into flagging and none needed individual exclusion either; the finding is uniform across the set.
- [x] **4.2** — Not performed, for any of the seven — there is nothing dead to flag. (For the record, for whenever a future pass *does* find one of these genuinely orphaned: `GridPresetPicker.tsx` (#35) and `resample.ts` are not actually the same convention. `GridPresetPicker.tsx` was abandoned-from-birth code with no working caller ever, flagged only in the decision log/`future-ideas.md`, no in-file comment — it's excluded from the `tsc -b` clean baseline instead. `resample.ts` is the closer analog to these seven files — real, previously-load-bearing code a migration made unreachable — and got an explanatory header comment at the top of the file itself, in addition to a decision-log mention. A genuinely dead file among these seven should follow `resample.ts`'s in-file-comment convention, not `GridPresetPicker.tsx`'s decision-log-only one.)
- [x] **4.3** — `sessions.ts` checked directly: its only importers are `vwap.ts`, `previousDayLevels.ts`, `premarketLevels.ts`, `camarillaPivots.ts`, `vpoc.ts` (`sma.ts`/`ema.ts` don't import it) — all five confirmed live per 4.1 above, so `sessions.ts` remains needed exactly as before; not a borderline case. `VolumeAvgIndicatorConfig` (`types/workspace.ts`) checked directly and confirmed to have **no dependency on `sessions.ts` at all** — it's an unrelated volume-bar-drawing feature, not session-aware math; the doc's own framing of it as a possible reason `sessions.ts` "may outlive" the rest doesn't apply, but doesn't need to, since the five real importers already settle the question.
- [x] **4.4** — Not applicable — nothing was flagged, so there is nothing to decide about deleting. No file under `frontend/src/indicators/` or `frontend/src/utils/indicators.ts` was touched.
