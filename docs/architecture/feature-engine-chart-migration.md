...
**Status:** Draft — not yet confirmed. Every stage below is a proposal to correct/approve, not a settled decision. Nothing in `confirmed-decisions.md` should be written until Stage 0 is explicitly signed off.
**Owner:** Saqib
**Companion documents:** [`system-design.md`](./system-design.md) (current architecture, incl. §1 non-goal, §4.5 Feature Engine, §4.11 chart-reading overlays), [`../decisions/confirmed-decisions.md`](../decisions/confirmed-decisions.md) (decisions #40/#41 — the client-side split this plan reverses; #45–#49 — the backend Feature Engine/Level Interaction Engine work this plan builds on), [`../roadmap/phase-roadmap.md`](../roadmap/phase-roadmap.md) (Phase 4 status).
...
- [ ] **2.1** — Decide A vs. B (or a hybrid: event-driven for 1m/5m since those are cheap and frequent, on-demand for 1h/1d since they're rare) and write the decision into `confirmed-decisions.md` with the reasoning, before any code.
- [ ] **2.2** — Implement whichever is chosen. Scope to SMA only again at this point — don't combine "new timeframe" and "new indicator" risk in the same step.
- [ ] **2.3** — Extend Stage 1's frontend path to request/accept non-1m timeframes.
- [ ] **2.4** — Verify against a real chart pane switched to 5m/15m/1h, each cross-checked against a manually-computed SMA over the same visible candles as a sanity check (not just "the number changed").
...
|---|---|---|---|
| 3.1 | EMA | Rolling state, per D3's resolution | Resolve D3 before starting — don't default to copying SMA's approach without checking the math holds |
| 3.2 | VWAP | Session-anchored accumulator (cumulative price×volume / cumulative volume), reset at session boundary via `MarketClock` | Needs a running total, not a fixed-size window — different state shape than SMA/EMA's deque |
