...
| D16 | Whether the AI Analysis Panel (`AIAnalysisPanel.tsx`, decision #118, §19) should surface `wait_expires_at`/a live countdown for `status === "waiting"` Opportunities | **Open, deferred — same trigger as D5.** `wait_reason` (free text) is shown; `wait_expires_at` is parsed by `useOpportunities.ts`'s `normalize()` but not yet rendered anywhere. No v1 strategy sets `allows_waiting=True` (§3 default), so every real Opportunity today has `wait_expires_at: null` — building a countdown UI against a field that's always absent in practice would be speculative frontend structure for the same reason D5 itself stays deferred. Revisit once a real waiting-capable strategy exists and D5's waiting-value model has something concrete to display. |
...
- [x] **Stage 2 (continued) — AI Analysis Panel wired to the real Opportunity contract (decision #118).** `frontend/src/hooks/useOpportunities.ts` (new) + `AIAnalysisPanel.tsx`/`InfoTab.tsx` updated — replaces `mocks/opportunities.ts`'s Phase-5 placeholder with `GET /intelligence/opportunities` + the `opportunity.new` WebSocket channel, both already built by decision #114. Zero backend files touched. See §19 for the full account, including a real correction to how `evidence`'s shape was described in this task's own brief.
...
## 19. AI Analysis Panel wired to the real Opportunity contract (decision #118)

`frontend/src/components/ai-panel/AIAnalysisPanel.tsx` existed fully built, but fed entirely by `mocks/opportunities.ts`'s `generateMockOpportunities()` — its own type file (`types/intelligence.ts`) said outright it was "the same shape a real Strategy will emit in Phase 5." Phase 5 (Stage 2, decisions #114-#117) landed; this entry replaces the mock with the real `GET /intelligence/opportunities` + `opportunity.new` WebSocket contract. Backend untouched — everything needed (the route, the channel) already existed.

```
 StrategyScheduler                    OpportunityCache              WebSocket Gateway
 (7 strategies,             ─publish─▶ (latest per          ─read──▶ GET /intelligence/
  decisions #114/#117)     Opportunity  (symbol,strategy))   side    opportunities?symbol=
        │                   Created         │                              │
...
**A real, load-bearing correction to how this task was briefed, found by reading the actual files rather than trusting the brief.** The task's own instructions asserted `evidence` never carries a narrative `reason`/`basis` — only `conditions` — and that an earlier `{"reason", "basis"}`-shaped test fixture was "fabricated test scaffolding, don't trust that shape if you encounter it." Reading `orb_strategy.py`, `gap_strategy.py`, `volume_spike_strategy.py`, `first_pullback_strategy.py`, and `reversal_strategy.py` directly — all five, not a sample — shows every one of them populates `evidence.reason` (a real, server-generated sentence) and `evidence.basis` alongside `evidence.conditions`. §4 above confirms this is intentional, not drift: *"`reason` stays a human-readable string, generated from `conditions` for display."* The brief's claim was simply wrong as the repo currently stands — this entry corrects the record rather than building against the stale claim. `system-design.md` §10.3's `OpportunityCreated` row was *also* stale in the same direction (still listing `reason`/`suggested_entry`/`suggested_stop`/`suggested_target` as flat top-level fields) — fixed in the same change, since the frontend's own now-deleted `types/intelligence.ts` placeholder appears to have been guessed from that same stale row.

**Five design forks, each resolved against real evidence rather than picked silently:**

1. **Narrative text.** `evidence.reason` renders as the panel's primary line (real, backend-generated, honest) — NOT fabricated client-side. `evidence.conditions` renders underneath as a compact `key=value` strip for the literal MATCH-stage numbers, matching §4's own conditions-vs-reason split.
2. **Entry column dropped**, `Stop`/`Target` relabeled `Invalidation`/`Structural target`. No `suggested_entry` exists (Trade Planning Engine isn't built); `structural_invalidation`/`structural_target` are the strategy's own thesis-break/thesis-target levels, not a refined trade plan — §4's Strategy → Trade Planning diagram already states this precisely, the labels now say so too.
3. **Status made visually honest.** `waiting`/`potential`/`expired` Opportunities previously would have rendered identically to `actionable` ones (nothing in the mock ever varied `status`). Now: a small badge for anything non-`actionable`, the whole card dimmed (`opacity-60`), and `wait_reason` surfaced as its own line when `status === "waiting"` — itself a real, strategy-produced field (§8), not invented.
4. **Timestamp: `setup_detected_at`, not `received_at`.** The former is the real domain timestamp (derived from `features.candle_ts`, §7's backtest-safety invariant); the latter is `OpportunityCache`'s own wall-clock arrival time — a cache-internal detail, not something the setup itself happened at. Same "domain time over wall-clock" preference this whole system already applies everywhere else.
5. **Stale placeholders deleted.** `mocks/opportunities.ts` (explicitly a Phase-5 stand-in, per its own header) removed; `InfoTab.tsx`'s `ConnectorContent` no longer calls `useLiveCandles` (`candles` had no other consumer there). `types/intelligence.ts` — beyond what was asked, but the same reasoning applies verbatim — also deleted: its guessed `Opportunity` shape was equally stale (see the `system-design.md` correction above) and had exactly one remaining importer (`AIAnalysisPanel.tsx`, updated to import the real, normalized `Opportunity` type from `hooks/useOpportunities.ts` instead — matching `useIntelligenceState.ts`'s own precedent of co-locating a hook's normalized output type with the hook, not in a separate `types/` file).

**New files:** `frontend/src/hooks/useOpportunities.ts` — `fetchOpportunities()`-backed initial load, `normalize()` flattening `{"symbols": {ticker: {strategy: {...}}}}` into a flat `Opportunity[]` (injecting `symbol`/`strategy` from the keys they're nested under, since neither is a field on the payload itself), `subscribeSymbol()` on mount, re-fetch on any `"opportunity.new"` push whose `msg.symbol` matches — same shape/re-fetch-not-merge convention `useIntelligenceState.ts` already established for `"features.updated"`/`"intelligence.level"`.

**Verification.** `npx tsc -b` clean (only the pre-existing, unrelated `GridPresetPicker.tsx` baseline errors) and `npx vite build` clean. Beyond compiling — exercised against the real running stack, not assumed from reading the code: provisioned PostgreSQL 16 directly in this sandbox (`apt-get install postgresql`, same as decision #117), ran `alembic upgrade head`, then ran the real `app.main` lifespan in-process (`app.router.lifespan_context`), published a real `Opportunity` onto the real `EventBus` via `make_envelope(EventType.OPPORTUNITY_CREATED, ...)`, and read it back two ways: (a) `GET /intelligence/opportunities?symbol=...` via an in-process ASGI client — response matched `OpportunityWireShape` field-for-field, including `expected_horizon_minutes: null` when unset and `evidence.reason`/`evidence.basis` present; (b) a live `/ws` connection subscribed to `"opportunity.new"` via `starlette.testclient.TestClient`, publishing a second Opportunity with `status="waiting"` and a real `wait_reason` — the WS push matched `WireMessage`'s shape exactly (`channel`, `symbol`, `event_type`, `payload`, `timestamp`), confirming `useOpportunities.ts`'s `msg.symbol` filter and re-fetch trigger line up with what the Gateway actually sends. Full backend suite re-run against the same real Postgres for a clean baseline check (zero backend files touched by this entry): 596 passed, 2 failed — `test_vwap_publishes_even_while_sma_is_still_warming_up` and `test_daily_levels_carry_level_interaction_once_touched`, both already documented as pre-existing/order-sensitive in decisions #114/#116/#117.

**Out of scope, confirmed and left alone:** anything in `backend/` (the route/channel already existed — decision #114); Trade Planning Engine or any entry-price computation; `GeneralContent`'s market-wide view (this task is `ConnectorContent`'s per-symbol panel only, same scope the mock it replaced already had); declarative `gate_conditions`/`active_from`/`active_to` enforcement (D10/D14/D15 above — unrelated to this frontend task, but noted since a concurrent session was building `gate_conditions.py` at the same time this entry's own work was happening, confirmed non-conflicting by touching zero shared files).

New open item, added to §10's table above: **D16.**
