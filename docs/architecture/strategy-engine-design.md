...
| D16 | Remove the 4 v1 strategies' now-redundant inline session GATE checks (`gap_strategy.py`, `momentum_strategy.py`, `volume_spike_strategy.py`'s explicit `MarketClock.is_regular_session()` calls; `orb_strategy.py`'s implicit self-gating via `minutes_since_open()`), now that decision #118 makes `StrategyScheduler`/`gate_conditions.py` the sole enforcement authority | **Open, deferred — flagged explicitly by decision #118, not done there.** These 4 checks predate decision #118's rule and are a tolerated, named exception to it, not a template for future strategies. Removing them is pure cleanup (the central gate already makes them behaviorally redundant — decision #117 confirmed all 4 already pass whenever the central check would), but each one's own test file currently asserts on the strategy's OWN GATE behavior directly, so removal is a real, if mechanical, change per strategy file, not a no-op — deliberately not bundled into #117/#118 to keep those changes reviewable on their own terms. Trigger: the next time any of these 4 files is touched for other work, or a standalone cleanup pass once a new strategy actually needs the sole-authority guarantee to hold with no exceptions. |
