...
**HTTP trigger route (decision #130) — the "optional API route" decision #128 deferred.** Everything above could only ever be exercised by writing Python directly against `BacktestRunner`'s constructor (`test_backtest_runner.py`'s own fixture-construction helpers were the only working example anywhere). `POST /backtest/run` (`backend/app/api/routes/backtest.py`) closes that gap — a thin caller in front of the exact same v1 slice diagrammed above, adding no new persistence or engine logic of its own:

```
   POST /backtest/run
   (strategy_name, symbol, scenario — all three required, no defaults)
              │
              ▼
   strategy_engine.scheduler.default_registry()   ◄── the same registry
   (validated against .name; 400 if unknown)          the live Scheduler
              │                                        itself builds from
              ▼
   backtest_runner.scenarios.SCENARIOS             ◄── a small, fixed set
   (validated against name; 400 if unknown)             of named fixture
              │                                          CSVs, checked in
              ▼
   BacktestRunner.run()      ◄── the unmodified v1 slice, diagrammed above
              │
              ▼
   BacktestRunResult, returned verbatim
   (run_id / sweep_id / outcomes_recorded / discarded_signals)
```

Two things worth stating plainly here rather than leaving them only in the route's own docstring, since both surprised this task's own build process and change how this route should actually be used:

- **Only 3 of the 7 v1 strategies can ever fire through this replay path today — a real, previously-undocumented structural finding, not a scenario-design shortfall.** `MarketState.volume_regime_score`/`volatility_regime_score` come from `FeatureEngine`'s `rvol`/`atr_14_pct`, which are populated exclusively from a `_daily_candle_cache` that only `_maybe_refresh_daily_levels()` fills, by calling `broker_registry.get_historical_provider().get_historical(symbol, "1d", ...)`. `EngineBackedReplayStateProducer` constructs a brand-new `FeatureEngine` with no historical provider wired in at all, and `BacktestRunner` never touches `broker_registry` — so both scores are structurally `0.0` for any replay, confirmed by direct execution, not just by reading the code (a 60-candle synthetic uptrend held `volume_regime_score` at `0.00` throughout). ORB/Gap/Volume Spike/Momentum all hard-gate their MATCH stage on `volume_regime_score >= threshold` — unreachable today, for any fixture, no matter how it's built. First Pullback/Reversal/VWAP gate MATCH only on `trend_score` and a real `LevelInteractionEngine` resolution, neither of which touches the daily-candle cache — each has its own scenario in `scenarios.py`, verified to genuinely fire by replaying it through this exact route. New open item **D18**, below.
- **Replay is measured at a real, fixed ~1 second of engine-settle time per candle** — not a request-processing cost, an actual `EngineBackedReplayStateProducer` floor confirmed by direct timing. All four scenarios are 120-140 candles specifically to keep this route's own response time to roughly two minutes rather than the 390 candles a full trading session would need; the route's own docstring states this is a deliberate v1 trade-off (no background-job/polling infrastructure added) rather than an oversight.
...
| D18 | `BacktestRunner`'s replay stack (`EngineBackedReplayStateProducer`) never wires a historical-data provider into `broker_registry`, so `FeatureEngine`'s `_daily_candle_cache` (backing `rvol`/`atr_14_pct`, and therefore `MarketState.volume_regime_score`/`volatility_regime_score`) stays permanently empty for any replay — found while building decision #130's trigger route | **Open, not attempted by decision #130.** Confirmed by direct execution, not just by reading the code: a 60-candle synthetic uptrend held `volume_regime_score` at `0.00` for the entire replay. Real, concrete consequence: ORB, Gap, Volume Spike, and Momentum all hard-gate their MATCH stage on `volume_regime_score >= threshold` (45.0 by default) — none of the four can ever produce a real `StrategyOutcome` through `BacktestRunner` today, regardless of how carefully fixture candles are built. First Pullback/Reversal/VWAP are unaffected (their MATCH stage never reads either score) and each has a real, verified-firing fixture scenario in `backtest_runner/scenarios.py`. Closing this would mean wiring a fixture daily-history provider into the replay stack specifically for `_maybe_refresh_daily_levels()` to find — real, separate work, deliberately not attempted here (decision #130's own task boundary was "a route + strategy lookup," not extending the replay engine's data sources) and genuinely bigger than it first looks: `broker_registry` is a process-wide singleton also used by live trading, so whatever's built needs to not risk clobbering a real connected provider mid-session, not just satisfy a backtest run in isolation. |
