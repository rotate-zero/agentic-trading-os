...
| D18 | Backtest replay writes real `symbols`/`daily_levels_state` rows under an arbitrary caller-supplied `symbol` label, with no `is_backtest` flag on either table — surfaced building decision #135's historical-provider seam, not resolved by it | **Open, deliberately not resolved by decision #135.** `FeatureEngine`'s real, unmodified `_reconcile_and_persist_daily_levels()` now actually runs during a backtest for the first time (decision #135 gave it a real historical provider to succeed against) — it was always going to write into these two SHARED tables the moment any historical-provider seam existed, regardless of which of decision #135's two fork options got picked; this isn't a consequence of the specific seam shape chosen. Two concrete symptoms, both confirmed by direct execution against a real Postgres: (1) a `symbol` label colliding with a real, live-tracked ticker lands synthetic backtest-derived daily levels in the same rows live trading reads; (2) re-running the identical `(symbol, scenario)` pair a second time silently reverts `volume_regime_score`/`volatility_regime_score` to `0.0` for that run, via `_maybe_refresh_daily_levels()`'s own pre-existing "restart-survival" short-circuit finding the first run's persisted row and skipping the raw-candle-cache population entirely. Three options were on the table when this was found (decision #135's own text): accept and document loudly (chosen for #135 — see `backtest.py`'s route docstring), have `BacktestRunner` clean up the rows it caused in a `finally` (reaches into a live-shared table's row lifecycle from backtest code, a bigger behavioral change than "wire a historical provider," not attempted), or add real `is_backtest`-style namespacing to `symbols`/`daily_levels_state` (the `strategy_outcomes`/`backtests` precedent, a genuinely larger schema change). Not decided here which of the latter two, if either, is worth doing — this row exists so the gap is found deliberately by whoever picks it up next, not rediscovered as a surprise. |
