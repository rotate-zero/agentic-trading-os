...
| D14 | Whether `StrategyScheduler` enforces `StrategyConfig.active_from`/`active_to` before calling `evaluate()` | **Open — a genuine conflict between what two parallel sessions were each told, not resolved here.** Track A's own delivery (decision #114) states plainly that Saqib confirmed `active_from`/`active_to` enforcement is explicitly OUT of scope for the Scheduler — every registered strategy is called regardless of its config's activation window, and `_default_registry()`'s `active_from` value has zero behavioral effect today. Separately, in this (Track B) session, Saqib instructed — while discussing this exact Scheduler's design — to "use the event/market timestamp rather than wall-clock `now` for activation-window evaluation where practical," which only makes sense if some activation-window comparison was expected to exist. These two instructions can't both be describing the same intended system. Flagged rather than silently picked either way — needs Saqib's direct clarification on which is correct before `scheduler.py` is touched on this point. If enforcement turns out to be wanted after all, the correct backtest-safe comparison is `features.candle_ts` (or `market_state`'s own timestamp) against `active_from <= ts` and (`active_to is None or ts < active_to`) — never `datetime.now()` inside the handler, per §7's existing invariant. |
...
- [x] **Stage 2, Track A — Strategy Scheduler built (decision #114).** `scheduler.py`: instantiates all 7 built strategies, subscribes `FeaturesUpdated` (caches the real payload — see decision #114 for why) and `MarketStateChanged` (the actual `evaluate()` trigger — also decision #114, a real correction found by testing, not the original plan), publishes `OpportunityCreated`. `main.py`/`conftest.py` wired for its own piece. **Stage 2, Track B — OpportunityCreated read-side (cache + `GET /intelligence/opportunities`) — also complete (decision #115).** Built in a parallel session, collided with Track A on decision number #114 (lost from the log when Track A's delivery was applied) and its own `main.py`/`conftest.py` wiring was flagged but never finished — both restored/completed in decision #115, verified end-to-end by running the real app lifespan. Declarative `gate_conditions` enforcement (§2b), `active_from`/`active_to` enforcement (open — see new D14, a genuine conflict between what two sessions were each told), and Opportunity Engine's ranking (§9) remain deliberately OUT of Stage 2's scope.
