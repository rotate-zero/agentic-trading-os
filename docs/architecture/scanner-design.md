...
**Revision note:** updated after a follow-up conversation that resolved IBKR's role in this plan (see new §9) — spread tightness (§2) and the Discovered-tier data source (§7) both now have a real answer instead of an open gap, on a timeline, not immediately. The rest of the plan (§1–§6, §8) is unchanged.
**Owner:** Saqib
...
1. **The actual Core-100 symbol list.** Not a technical question — needs Saqib's own criteria (liquidity, sector spread, personal watchlist history, etc.).
2. **ATR-normalizing gap/session-change, vs. scoring them raw** — §2's proposal, not yet validated against real behavior.
3. **Weight defaults (1.0/1.0/1.0)** — fine to ship and tune empirically, or is there a strong prior for weighting one input higher?
4. **Build-now-on-Finnhub vs. wait-for-IBKR** (§9) — decides whether anything gets built out of this doc immediately or later.
5. **IBKR parallel vs. replace Finnhub** (§9) — only urgent once the answer to #4 is "build now," since it determines what v2's upgrade path looks like.

Dropping spread tightness from v1 is no longer an open question — §9 settled it as a deliberate two-phase plan, not a permanent gap. The Finnhub-100-symbol concurrency check is only relevant if #4 resolves to "build now"; moot if Core tier ships on IBKR from day one. None of the above blocks writing the code for §1–§6 — they're tuning/sequencing questions the code's config surface already accommodates. They do need answers before this becomes a confirmed decision entry.
