...
**Version:** 2.11 (Pre-Implementation) — §1, §4.5, §4.11 updated for the chart→Feature Engine migration (`confirmed-decisions.md` #50–51; full plan: `feature-engine-chart-migration.md`); §4.5 also notes the Daily Levels direction lock (`confirmed-decisions.md` #59; full plan: `daily-levels-design.md`); §4.5/§4.8 also note the Strategy Engine direction lock (`confirmed-decisions.md` #87–88; full plan: `strategy-engine-design.md`)
**Status:** Phase 2 kickoff — Event Bus dispatch lanes and shared update-policy utility added
**Owner:** Saqib
**Companion documents:** [`trading-intelligence-architecture.md`](./trading-intelligence-architecture.md) — how the system thinks (market state, context, strategy, decision logic). [`feature-engine-chart-migration.md`](./feature-engine-chart-migration.md) — the stage-by-stage plan moving chart-drawn indicators onto Feature Engine as their single source (confirmed decision #50), tracked there rather than re-derived from this doc each session. [`daily-levels-design.md`](./daily-levels-design.md) — the stage-by-stage plan for the Daily Levels support/resistance clustering indicator (confirmed decision #59), all four stages built (decisions #60–#65). [`feature-engine-indicator-expansion.md`](./feature-engine-indicator-expansion.md) — the stage-by-stage plan for ATR/Session Change/Gap/Regression/KAMA (confirmed decision #67), all five families built (decisions #67–#71). [`strategy-engine-design.md`](./strategy-engine-design.md) — Strategy Engine internals, versioned `StrategyConfig`, and the `Opportunity` schema extension below (confirmed decisions #87, #99), ORB (Stage 1's first strategy) built. As of `split-strategy-engine-design-doc` (decision #142), three parts of that plan live in their own sibling files: [`backtest-runner-design.md`](./backtest-runner-design.md) (formerly its §7), [`strategy-engine-open-decisions.md`](./strategy-engine-open-decisions.md) (formerly its §10), [`strategy-engine-build-history.md`](./strategy-engine-build-history.md) (formerly its §14–§18). [`../decisions/future-ideas.md`](../decisions/future-ideas.md) — concepts raised and deliberately deferred, with reasoning intact, so they aren't lost or re-argued from scratch. [`../decisions/confirmed-decisions.md`](../decisions/confirmed-decisions.md) — the running settled-decisions log. [`../roadmap/phase-roadmap.md`](../roadmap/phase-roadmap.md) — phased delivery plan and exit criteria. [`execution-engine-design.md`](./execution-engine-design.md) — design for the Execution Engine and Portfolio State — approved in principle (simulated-venue automatic path first) and amended by decision #170; verified as-built inventory, the implementation specification for that slice, and the forks still open (EX-5, EX-8, EX-9, EX-11–EX-14). This doc explains how the system is built. Read all of them; they're deliberately kept separate. See [`../README.md`](../README.md) for how the whole `docs/` tree is organized.
...
**Finnhub/Polygon connection status reaches the frontend for the first time (decision #143).** `GET /finnhub/status` and `GET /market-data/status` (both existing, working routes — `finnhub_data.py`/`market_data.py`) had zero UI representation before this; only `curl` could ask "is either provider actually connected right now." A small always-visible indicator now sits in `App.tsx`'s `<header>` (both the full workspace and the popped-out window shell):
...
**Implementation status (confirmed decisions #45, #51, #52, #53, #56, #57):** SMA is the first indicator actually computed here — `app/feature_engine/{indicators/,engine.py}` — published as `FeaturesUpdated` on every `1m` `CandleClosed`, in-memory rolling window per `(symbol, timeframe)` seeded from `candle_store`/`candle_aggregator` on first sight of a symbol+timeframe, restart-safe by design (same "rebuilt from persisted history" shape as Market State — §4 of the companion doc). **EMA is also computed (decision #52)**, reading from the same rolling window — a full-window recompute (seed with SMA of the window's oldest bars, then recurse forward) rather than true incremental recursion, since EMA has no exact finite-window equivalent to SMA's; see `indicators/ema.py::ema()`'s own docstring for the convergence math and the stricter warm-up (`period * seed_multiplier` closes, not just `period`) this implies. **VWAP is also computed (decision #53)** — a genuinely different shape from SMA/EMA, not a third rolling-window variant: keyed by symbol alone rather than `(symbol, timeframe)`, since it's a session-level statistic that should read identically regardless of chart timeframe, computed once from `1m` bars and attached to every timeframe's `FeatureSet` on that same close; a monotonically growing session accumulator, not a sliding window, reset via `MarketClock.is_regular_session()`/`session_bounds()`, excluding pre-market/after-hours volume entirely (matching `frontend/src/indicators/vwap.ts`'s own convention). **PDH/PDL/PDC, Camarilla, pre-market H/L, and VPOC are also computed (decisions #56, #57)** — also symbol-keyed like VWAP (none vary by chart timeframe), but genuinely different from VWAP in shape too: PDH/PDL/PDC is a FIXED value for the whole of today, recomputed once per ET-calendar-day rollover (not accumulated candle-by-candle) from the most recent day with persisted data — skipping weekends/holidays automatically, since no candles were ever recorded on a day the market didn't open; Camarilla is a pure function of that same PDH/PDL/PDC, computed once alongside it, never re-derived; pre-market H/L accumulates like VWAP but gated to `PRE_MARKET` specifically, and — unlike VWAP — stays published (frozen) through the rest of the day once pre-market ends, rather than disappearing; **VPOC (decision #57) turned out to need none of the live-histogram machinery D5 originally anticipated** — the frontend's own VPOC is scoped to the previous day only, the exact same bounded dataset PDH/PDL/PDC already fetch, so it's computed alongside them from the same rows, one more scalar in the same flat dict. All of these publish under their own `FeatureSet.features` keys, which is the ENTIRE integration needed for Level Interaction Engine (§4.8) to start tracking touch/reject/conquer against them — confirmed directly, not assumed, by re-reading that engine's code before building any of this. Everything else in the paragraph above (opening range, average volume, trend slope, signed-volume imbalance) is still the target shape, not yet built — Gap %, Session % Change, ATR, and Relative Volume are the four exceptions, now actually computed (decisions #67, #68, #69, #71, see below). **`5m`/`15m`/`1h` SMA/EMA are also computed (decision #51)** — event-triggered off the same `1m` `CandleClosed`, the moment it completes a higher-timeframe bucket (`candle_aggregator.completes_bucket()`), not a separate poll or its own event source; cold-start history for those timeframes comes from `candle_aggregator.aggregate_from_recorded()` rather than `candle_store` directly, since `CandleRecorder` only ever persists `1m` rows. `FeatureSet` gained a `close` field in decision #46, for the Level Interaction Engine below (§4.8) — the first real consumer of this output. **`FeatureSet` gaining `open`/`high`/`low`/`volume` alongside `close` is now built, 1m only (decision #99)** — Strategy Engine's ORB needed real candle-shape structure (a true opening range needs wick highs/lows, not just closes) and this was the pre-scoped, purely additive fix waiting for a real consumer; see `strategy-engine-design.md` §8 / `strategy-engine-build-history.md` §14 for the full reasoning, including why aggregated `5m`/`15m`/`1h` `FeatureSet`s still leave these `None` (true aggregated-bucket OHLC isn't tracked anywhere, and passing through the last constituent 1m candle's own OHLC would misrepresent the aggregated bar rather than just be incomplete) and the separate, still-unwired `PriceSnapshot` feed for the currently-forming bar. **The Chart is a real consumer of every indicator above** — SMA/EMA/VWAP as continuous overlay lines (decision #54, via `GET /intelligence/series`), and PDH/PDL/PDC/Camarilla/pre-market H/L/VPOC as horizontal price lines (decision #58, via the already-existing `GET /intelligence/state` — no new endpoint needed, since these are single scalars, not per-candle series) — rather than `frontend/src/indicators/`'s independent local math for any of them; see §4.11 for the Chart-side detail on both paths. **Daily Levels (support/resistance zones clustered from 180 days of 1D open/close prices) is built — all four stages complete (decisions #60–#65)** — this sentence previously read "has its direction locked (decision #59) but is not yet built," true when originally written, stale since those stages landed; corrected here. See `daily-levels-design.md` for the full build account. It is the first Feature Engine indicator with a genuine external-provider (Polygon) dependency and the first with a variable-length `daily_levels` list on `FeatureSet` rather than a flat-dict scalar. **Session % / $ Change and Gap % / $ are also now computed (decisions #67, #68, Stage 1 of `feature-engine-indicator-expansion.md`)** — both symbol-keyed like VWAP/PDC, both reference the same `pdc` PDH/PDL/PDC already compute, but genuinely different from each other in shape: Session % Change is a pure function of `close` + `pdc` with no state of its own, continuously updating through pre-market/regular/after-hours alike; Gap captures `regular_open` ONCE on the first candle where `MarketClock.is_regular_session()` turns true for the day and freezes it for the rest of the day — same "established once, frozen" shape pre-market H/L's own freeze uses, restart-safe via the same backfill-from-persisted-history pattern VWAP/pre-market H/L already establish. **ATR(1D,14)/ATR% is also now computed (decisions #67, #68, #69, Stage 2 of `feature-engine-indicator-expansion.md`)** — Wilder's classic ATR over the last 14 complete daily bars strictly before today, recomputed once per `(symbol, ET day)` and frozen, ATR% against that same frozen daily-bar close rather than the live price. Reads from a shared `self._daily_candle_cache`, extracted out of Daily Levels' own fetch specifically so ATR causes zero additional Polygon calls — proven directly in the test suite (`fake.call_count == 1` with both `atr_14` and `daily_levels` present in the same `FeatureSet`), not just architecturally argued. **Configurable-timeframe Linear Regression and configurable-timeframe KAMA are also now computed (decisions #67, #68, #70, Stages 3+4 of `feature-engine-indicator-expansion.md`)** — all five feature families from the original expansion brief are built. Regression and KAMA introduced this engine's first per-indicator-family `(timeframe, period[, ...])` config shape (`feature_engine_regression_configs`/`feature_engine_kama_configs`), since SMA/EMA's flat period list can't express "compute this only for 1m and 5m, with independently-configurable periods per timeframe" — and are the first two families where `_apply_close` checks whether a given `(indicator, timeframe)` combination is even configured before computing anything, rather than applying uniformly to every timeframe that fires. Both read from the SAME shared per-`(symbol, timeframe)` rolling window SMA/EMA already use, sized to the largest of all four families' needs. Slope normalization for both uses local intraday volatility (the same window's own close stdev), not the ATR family, avoiding an unstated cross-timeframe unit conversion. **Relative Volume is also now computed (decision #71, `feature-engine-indicator-expansion.md` §13, added separately after the original five-family brief closed out)** — a time-of-day-normalized proxy comparing today's regular-session volume so far against the average of the last 5 complete trading days, reusing the SAME shared daily-candle cache (a third consumer now) and, as a byproduct, `_update_vwap` was refactored to publish `session_volume` as its own feature key rather than keep it purely internal.
...
**Design pass (approved in principle; amended by decision #170):** [`execution-engine-design.md`](./execution-engine-design.md) records where this section and the as-built code diverge and how the first slice resolves it — Portfolio State owns position accounting, in-flight orders, and daily P&L as a cache over an authoritative database ledger (rebuilt from it at startup), `PositionClosed` is published on the critical lane only after the closure is committed, and the lane itself provides ordering and handler-failure isolation, not persistence or crash recovery. §4.8's row that has Position Monitor emit `PositionClosed` is superseded for this slice by that design.
...
| Performance Intelligence | `PositionClosed`, `feature_snapshots`, `market_events` | strategy reweighting signal | `strategy_outcomes` |

**Implementation status (confirmed decision #46):** Level Interaction Engine is the first module in this table actually built and running — `app/trading_intelligence/level_interaction_engine.py`. Generic by construction: it runs the same touch/holding/rejected/conquered state machine against every key `FeatureSet.features` carries (currently `sma_9`/`sma_20`/`sma_50` — decision #45), so it needs no changes when EMA/VWAP/pivot levels start publishing later — and, as of decision #51, needed no changes to start tracking `5m`/`15m`/`1h` SMA either, since it was already generic over `timeframe`, not hardcoded to `1m`. **As of decisions #85/#86, every non-price-coordinate key Feature Engine currently publishes is excluded from tracking** (`_is_excluded_from_level_tracking`, checked ahead of the generic loop in `_process_one`): the SMA/EMA slope family (`sma_{period}_slope`/`_r2`/`_slope_pct`/`_slope_angle`, `ema_` equivalents — decision #85), ATR/ATR% (excluded wholesale — a magnitude, never a price coordinate), Gap, Session Change, RVOL, Premarket Volume Ratio, the two session volume totals, and Regression/KAMA's own derived slope/deviation/r2/dist/efficiency byproducts (decision #86) — none of these is a price, so none belongs in touch/holding/rejected/conquered tracking against `close`. `kama_{period}` and, as of decision #86's own deliberately-argued call, `regression_{period}_value` are the one genuinely subtle case: both publish a computed price sitting on the same scale as `close` (unlike the derived byproducts sitting right next to them in the same dict), so both stay tracked, following the same "reference line kept, its own delta excluded" split for both families. **The list of families still flagged-but-not-yet-addressed, non-empty since decision #83's own write-up first raised it, is now empty** — every key Feature Engine currently publishes is either a tracked price-scale reference or an explicitly excluded non-price quantity; a genuinely NEW future indicator family would still need its own explicit inclusion/exclusion call when it's added, same as every family above got, but nothing already-published is left undecided. **Market State Engine and Context Engine are also built** (decisions #92/#93/#96/#97 — this line previously read "still the target shape only, not yet built," true when originally written, stale since those decisions landed; corrected here per decision #98). Both now also expose a synchronous `get_snapshot(symbol=None)` read alongside their event publication (decision #98, M4) — `app/market_state_engine/engine.py`, `app/context_engine/engine.py`, surfaced via `GET /intelligence/market-state` and `GET /intelligence/context`. **Strategy Engine's first strategy, ORB, is also now built (decision #99)** — `app/strategy_engine/{base_strategy.py,orb_strategy.py}`, consuming `get_snapshot()`/`FeaturesUpdated` exactly as decision #98 prepared for. Everything below Strategy Engine in this table (Opportunity Engine, Decision Engine, Governor, Trade Planning Engine, Position Monitor) remains the target shape only, not yet built.
...
**Extension now built (`strategy-engine-design.md` §3–4, decisions #87, #99):** `strategy` gains a required `version` (immutable `StrategyConfig` identity — a threshold change mints a new version, never edits one in place). `reason` (kept, for display) gains a structured `evidence` sibling — `{"conditions": {...the literal MATCH-stage values...}, "reason": "...", "basis": "live"|"closed"}` — so Performance Intelligence can later query the actual signal values a trade fired on, not just read a sentence. `suggested_stop`/`suggested_target` rename to `structural_invalidation`/`structural_target`: the price at which the strategy's own thesis is falsified, which Trade Planning Engine (`trading-intelligence-architecture.md` §11) treats as a required starting point to refine, not a number it's free to discard. Deliberately no `symbol` field on `Opportunity` itself — same convention `FeatureSet`/`MarketState` already use; it lives on the `EventEnvelope` once published as `OpportunityCreated`.
...
**Design pass (approved in principle; amended by decision #170):** [`execution-engine-design.md`](./execution-engine-design.md) records where this section and the as-built code diverge and how the first slice resolves it — the Execution Engine talks to venues through a new narrow `OrderVenue` port and an `execution` registry role (`BrokerAdapter`, whose job is market-data connectivity, is not enlarged); every order has a stable client-order ID and is persisted before it is sent; fills are deduplicated and persisted before `OrderFilled` is published; restart recovery reconciles non-terminal orders with the venue; and the slice fails closed for any `execution_mode` other than `simulated`.
...
**Chart-reading overlays (moving averages, session levels, volume averages) have been migrated onto Feature Engine as their single source — no longer a permanently separate, client-side-only category (confirmed decision #50, superseding the framing this paragraph carried under decisions #40/#41; full stage-by-stage plan: `feature-engine-chart-migration.md`).** The Chart widget draws two kinds of backend-sourced lines, each with its own instance model and its own resolver function — not one system, because the rendering shapes are genuinely different, confirmed by reading `types/workspace.ts`/`ChartWidget.tsx` directly rather than assumed from one indicator's pattern carrying over to the other: **overlay indicators** (`PriceIndicatorInstance` — SMA/EMA/VWAP) are continuous `{time,value}[]` lines resolved by `computePriceIndicator()`; **horizontal levels** (`HorizontalLevelInstance` — PDH/PDL/PDC, all nine Camarilla levels, pre-market H/L, VPOC) are single fixed price values resolved by `computeHorizontalLevel()` and drawn via `series.createPriceLine`. (Backend-pushed `ChartObject`s — §4.10, server is the source of truth, e.g. an AI-placed annotation — are a third, older category, unrelated to this migration.) Both resolver functions' local math still lives in `frontend/src/indicators/` (`sma.ts`, `ema.ts`, `vwap.ts`, `previousDayLevels.ts`, `premarketLevels.ts`, `camarillaPivots.ts`, `vpoc.ts`, plus a shared `sessions.ts`) — until decision #40 there was no backend Feature Engine to point any of these at, so they all computed independently, quietly duplicating the same math Feature Engine now also computes (§4.5), exactly the failure mode §2's "compute once, consume everywhere" principle exists to rule out.

**Current state: both paths are genuinely running, not target-state description.** `computePriceIndicator` (decision #54) sources SMA/EMA/VWAP from `GET /intelligence/series` (backfill) plus live `features.updated` pushes (`useFeatureEngineSeries.ts`). `computeHorizontalLevel` (decision #58) sources the other seven from `GET /intelligence/state` — no new endpoint needed there, since a horizontal level is one current scalar, not a series, so the ALREADY-built, zero-DB-I/O `useIntelligenceState.ts` (decision #47/#48) already served it; a thin new hook, `useFeatureEngineLevels.ts`, just extracts the flat lookup `computeHorizontalLevel` needs from it. Both resolvers use the same pattern either way: when Feature Engine has a value for the exact instance configured (matching period/timeframe for overlays; matching level type for horizontal lines), that's used directly; when it doesn't (a non-standard SMA period, a timeframe outside `1m`/`5m`/`15m`/`1h`, or no previous trading day within the configured lookback yet), it falls back to the original local computation — unchanged — with `" (local)"` appended to the label, so the fallback stays visible rather than silently indistinguishable from a backend-sourced line. None of the seven `frontend/src/indicators/` files above are retired by any of this — they're the real, load-bearing fallback path for both resolvers, still called whenever the backend can't serve a given instance, not dead code.
...
**HUD text box — a floating, multi-line Feature Engine readout pinned to the top of the chart pane, independent of the two indicator systems described above.** `SubWindowConfig.hud: HudConfig` is a third, deliberately distinct shape from `priceIndicators`/`horizontalLevels`: not a line drawn on the price series at all, but an on-chart overlay `<div>` (`components/chart/HudBox.tsx`, same absolute-positioned mechanism `TimerBadge.tsx` already established) showing up to 3 independently-toggleable lines, each a free-ordered mix of literal text and live variables rather than a fixed template. Its 8 variables (`utils/hud.ts`'s `HUD_VARIABLES`) read gap %/$, session %/$ change, ATR(14) raw and %, RVOL, and session volume straight off the existing `useIntelligenceState` snapshot via a dedicated `useHudFeatures.ts` hook — no new backend endpoint, no new WebSocket channel. Background (hex + opacity) and text (hex) are independently stylable via the existing `ColorField` control; position defaults to top-left specifically to avoid the Timer badge's own top-right corner, though both are freely repositionable. Has no chart-series dependency at all, unlike every other indicator in this section — it never calls `createPriceLine`, so it needed no `chartStyle` awareness when decision #73 above was implemented, and has none. See `../decisions/confirmed-decisions.md` #75.
...
Core tables: `symbols`, `candles` (natively partitioned by month, sub-partitioned by timeframe), `trades` (gains an `origin: auto|manual` column and a `pending_confirmation` status once manual trading lands — `trading-intelligence-architecture.md` §18), `orders`, `positions`, `ai_decisions`, `strategy_outcomes` (renamed from `strategy_performance` — decision #89, one row per closed trade; full shape in `strategy-engine-design.md` §5; **built — migration 0008, decision #120**, this clause previously read "no migration yet — the table itself is still not built," true when originally written, stale since decision #120 landed; corrected here — real backtest-derived rows exist since decision #128, zero live-trading rows since no Execution Engine/Position Monitor exists yet to write one; decision #98's `app/trading_intelligence/state_snapshot.py` provides the real read contract its `market_state_at_entry/_at_exit`/`context_at_entry/_at_exit` fields draw from), `workspace_layouts`, `watchlists`. Alembic manages migrations from day one so schema drift never becomes a manual-SQL problem.
...
Deferred, not created in v1 migrations (names reserved so the schema doesn't fight the future): `replay_sessions`. `backtests` (shape locked — decision #89; full field list in `backtest-runner-design.md` §7) is no longer deferred either — built per decision #120, migration `0008_strategy_outcomes_and_backtests.py`, written by Backtest Runner v1 (decision #128) and read via `GET /intelligence/backtest-runs` (decision #136); this line's own "not yet created" claim was stale and is corrected here. `symbol_fundamentals` (decision #90's shape) is no longer deferred — built per decision #96, migration `0006_symbol_fundamentals.py`; full field list still in `trading-intelligence-architecture.md` §5, `sector` documented there as permanently unpopulated (Finnhub provides one classification field, not a sector-and-industry pair).
...
| `PriceSnapshot` | `timeframe`, `open`, `high`, `low`, `close`, `volume`, `candle_ts` — same shape as `CandleClosed` deliberately (confirmed decision #72), but describes the CURRENTLY FORMING bar, not a closed one. Published by `LiveTickRelay` (`app/services/live_tick_relay.py`), throttled to at most one per symbol per 5s, only for symbols in its small (max 8), explicitly-set active set — not a general-purpose event every symbol receives. |
| `FeaturesUpdated` | `timeframe`, `candle_ts`, `close` (confirmed decision #46 — makes this self-contained for a consumer needing both a level value and the raw price), `open`, `high`, `low`, `volume` (decision #99 — **1m only**; `None` on aggregated `5m`/`15m`/`1h` FeatureSets, see `strategy-engine-design.md` §8 / `strategy-engine-build-history.md` §14), `features: {...}` — **as built (decisions #45, #51, #52, #53, #56, #57):** `{sma_9, sma_20, sma_50, ema_9, ema_20, vwap, pdc, pdh, pdl, cam_pp, cam_r1..r4, cam_s1..s4, pmh, pml, vpoc}` (SMA/EMA periods configurable) on `1m`, `5m`, `15m`, and `1h` — `vwap`/`pdc`/`pdh`/`pdl`/`cam_*`/`pmh`/`pml`/`vpoc` are all the SAME value across all four timeframes, per decisions #53/#56/#57. **Also as built (decisions #67-#71, `feature-engine-indicator-expansion.md`), corrected here from a previously-stale "not yet computed" listing:** `atr_14, atr_14_pct` (daily, decision #69), `gap_pct, gap_dollars` (decision #68 Stage 1), `session_pct_change, session_dollar_change` (decision #68 Stage 1), `rvol, session_volume` (decision #71), `regression_{period}_value/slope/deviation/r2[/slope_norm]`, `kama_{er_period}[/_slope/_dist/_dist_pct/_er/_slope_norm]` (decisions #67/#68/#70, configurable `(timeframe, period[, ...])` pairs, 1m+5m by default — NOT the uniform SMA/EMA fan-out). **Also as built (decision #83):** `sma_{period}_slope, sma_{period}_r2[/_slope_pct/_slope_angle]` and `ema_{period}_slope, ema_{period}_r2[/_slope_pct/_slope_angle]` — same periods/timeframes as `sma_{period}`/`ema_{period}` above, slope's own lookback tied 1:1 to each period rather than separately configured; normalized against the CURRENT sma/ema value (%/bar), not ATR — an intraday slope against ATR's fixed daily range is the same unstated cross-timeframe conversion decision #67 already ruled out for regression/KAMA; `slope_angle` is `arctan(slope_pct)` in degrees, calibrated so 45° = exactly 1%-of-value per bar. `opening_range_high`/`opening_range_low` deliberately NOT added here despite being named in this paragraph's original target-shape list — decision #99 (Saqib's call): ORB's opening range is tracked privately inside `orb_strategy.py`, never published to `FeatureSet` or anywhere else. **Also as built (decisions #100/#101, `premarket-accumulator-design.md`), missing from this table until decisions #100-102 backfilled a gap of their own:** `vwap_ext, session_volume_ext` (one continuous accumulator from 4:00am, never resetting at 9:30am unlike `vwap`/`session_volume` — decision #100) and `premarket_volume_ratio` (decision #101, published only while `current_session() == Session.PRE_MARKET`, mutually exclusive with `rvol` by session, never both on the same `FeatureSet`). `avg_volume_20d`, `trend_slope` remain not yet computed (`trend_slope` is superseded by `sma_*_slope`/`ema_*_slope`/`regression_*_slope`/`kama_*_slope` above, not a separate field still owed) |
...
| `MarketStateChanged` | Two payload shapes, same event type, distinguished by `envelope.symbol` (decision #91: "no new EventType needed"). Per-symbol shape (`envelope.symbol` = a real ticker): `timeframe`, `candle_ts` (same convention as `FeaturesUpdated`), `trend_score`, `volatility_regime_score`, `volume_regime_score`, `vwap_relationship_score`, `acceleration_score` (Trend's own rate of change; `None` on a symbol's first-ever recompute) — all 0–100 except `acceleration_score` (decision #91). No `dimension`/`value`/`duration_seconds`/`strength`/`confidence`/`previous_value`/`changed_at` — that pre-score-first shape was superseded by #91 before being built; duration/confidence/tags are deferred to the band/classification system (`future-ideas.md` #22). `session_type_score` dropped from this build entirely (decision #93) — doesn't fit the directional/magnitude shape the other four use; flagged to revisit, not designed away permanently. Cross-symbol shape (`envelope.symbol == "__MARKET__"`): `timeframe`, `candle_ts`, `spy_direction_score`, `qqq_direction_score`, `iwm_direction_score`, `trend_alignment_score`, `risk_on_score`, `qqq_leadership_score`, `iwm_confirmation_score` (decision #91's `CrossSymbolState`, built decision #97 — M3). Confirmed decision #93 (per-symbol) and #97 (cross-symbol) — built: `app/market_state_engine/engine.py`, `app/market_state_engine/scoring.py`. Routed to WebSocket channel `intelligence.market-state` (decision #146, `app/api/websocket/channels.py`) — same one-channel-both-shapes pattern as `ContextChanged` below, except here `envelope.symbol` is never unset: a subscriber tells the two shapes apart by comparing it to the literal sentinel `"__MARKET__"`, not by checking for null/absent. **`GET /intelligence/market-state` (decision #98) separately reached the frontend for the first time (decision #147), built and packaged before the `intelligence.market-state` WebSocket channel above existed** — confirmed by reading `channels.py` directly at that time; originally fetch + 5s poll (`useMarketState.ts`). **As of decision #151, `useMarketState.ts` is wired onto that channel** — WebSocket-primary, poll fully removed rather than kept as a fallback (unlike `ContextChanged`'s own decision #126 resolution below), since this engine's own recompute cadence (~1s floor/~10s ceiling per symbol, ~1s floor/~4s ceiling cross-symbol) makes a dropped/reconnecting session self-heal within seconds — the same reasoning `useOpportunities.ts`'s own WS-only design already rests on. Per-symbol scores land in `AIAnalysisPanel.tsx`, the cross-symbol composite in `InfoTab.tsx`'s `GeneralContent`, same market-wide-vs-per-symbol split decision #125 used for Context. |
| `ContextChanged` | Two shapes, decision #96: **market-wide** (`symbol` unset) — `providers: {calendar: {...}}`, published on `ContextEngine.evaluate_all()`'s session-boundary loop. **Per-symbol** (`symbol` set) — `providers: {fundamentals: {...}, news: {...}}`, published on `ContextEngine.evaluate_for_symbol()`'s 15-minute-per-symbol timer. `gap`/`levels`/`volatility_regime` never existed as real providers — decision #90 cut them before either was built. Confirmed decisions #92 (market-wide path, `CalendarProvider`) and #96 (per-symbol path, `FundamentalsProvider`/`NewsFlagProvider`) — built: `app/context_engine/engine.py`, `app/context_engine/providers/`. Routed to WebSocket channel `intelligence.context` (decision #126, `app/api/websocket/channels.py`) — both shapes share the one channel, the frontend distinguishes them by the envelope's `symbol`. |
| `OpportunityCreated` | **Stale row, corrected here** — this table's own §10.2 rule ("if code and doc disagree, fix the doc") applies; this row hadn't been touched since before §4's rename (decision #99, restated `strategy-engine-design.md` §4). As actually built: `strategy`, `version`, `direction`, `confidence`, `structural_invalidation` (was `suggested_stop`), `structural_target` (was `suggested_target`), `expected_horizon_minutes` (optional, decision #107), `evidence: {conditions: {...}, reason, basis}` (was a flat top-level `reason` — see §4's full reasoning for why it's nested), `status`/`wait_reason`/`wait_expires_at` (decision #88, §8), `setup_detected_at`/`confirmed_at`/`decided_at`. No `suggested_entry` anywhere — Trade Planning Engine, which would compute one, isn't built (§4's Strategy → Trade Planning diagram). No `symbol` on the payload itself, same convention as `FeatureSet`/`MarketState` — it's on the envelope. Found and fixed while wiring the frontend AI Analysis Panel to the real contract (decision #118) — the frontend's own placeholder type (`types/intelligence.ts`, since deleted) had independently guessed this same stale shape, suggesting this row was the source both drifted from. |
...
**As-built note (decision #146):** `MarketStateChanged` reaches the frontend the same way `ContextChanged` does (decision #126) — no new publish, only a routing entry. Cross-component flow:

```
 MarketStateEngine._worker_loop()
   │
   │  publish(MarketStateChanged)               ← already existed (decisions #93/#97);
   │                                               no change made here
   ▼
 EventBus (§4.4)
   │
   │  same envelope, both shapes reach every
   │  WebSocket Gateway subscriber path
   ▼
 WebSocket Gateway (§4.12) — app/api/websocket/channels.py
   │
   │  EVENT_TO_CHANNEL[MARKET_STATE_CHANGED]     ← the one line this delivery adds
   ▼
 "intelligence.market-state" channel
   │
   ▼
 (future) frontend subscriber — no consumer wired yet, same as the channel
          existing on its own before any hook subscribes to it
```

Internal flow inside the changed module (`channels.py` itself does no branching — the two shapes are indistinguishable to it and reach the same channel; a subscriber does the branching):
...
```
 EventEnvelope in
   ...
   ▼
 EVENT_TO_CHANNEL[event_type]  →  "intelligence.market-state"
   │
   ▼
 re-published on that channel, envelope.symbol untouched
   │
   ├─ envelope.symbol == "<TICKER>"   → per-symbol MarketState shape
   └─ envelope.symbol == "__MARKET__" → cross-symbol CrossSymbolState shape
        (subscriber's own responsibility to branch on this; channels.py
         does not inspect symbol at all — same as it already doesn't for
         ContextChanged's null-vs-set convention)
```

**As-built note (decision #151):** `useMarketState.ts` now consumes the `intelligence.market-state` channel the `market-state-changed-websocket-channel` delivery above opened, mirroring `useContextSnapshot.ts`'s decision #126 upgrade — but fully replacing its own poll rather than keeping one as a fallback (reasoning: this hook's own docstring, and the row above). Cross-component flow:

```
 MarketStateEngine._worker_loop()
   │
   │  publish(MarketStateChanged)          ← unchanged, already existed
   ▼
 EventBus (§4.4) → WebSocket Gateway (§4.12) → "intelligence.market-state"
   │                                             (both already existed —
   │                                              decision #146)
   ▼
 workspaceSocket (frontend/src/services/websocket-client.ts)
   │
   │  one shared connection, re-published per-channel to every
   │  subscribed handler — no change made here
   ▼
 useMarketState.ts's onUpdate handler        ← the new subscriber this
   │                                            delivery adds
   ▼
 load() → GET /intelligence/market-state (fetchMarketStateSnapshot)
   │        (unchanged REST call — push is an invalidation signal only,
   │         never a second payload-normalization path)
   ▼
 setSymbolState() / setMarket()  →  AIAnalysisPanel.tsx / InfoTab.tsx
                                     (unchanged consumers — hook's public
                                      return shape did not change)
```

Internal flow inside the changed module (`useMarketState.ts` itself — replaces the old fetch+poll effect body):

```
 effect runs (mount, or `symbol` argument changes)
   │
   ├─ load()                                  ← unconditional initial fetch
   │
   └─ workspaceSocket.subscribe(
        "intelligence.market-state", onUpdate)
        │
        │  on every MarketStateChanged push reaching this channel:
        ▼
      msg.symbol == "__MARKET__" ? ──yes──► load()  (cross-symbol changed)
        │no
        ▼
      msg.symbol == symbolRef.current ? ──yes──► load()  (this symbol changed)
        │no
        ▼
      ignore (a different symbol's MarketState — not this instance's concern)

 effect cleanup (unmount, or `symbol` changes again):
   unsubscribe()   ← no interval to clear anymore; POLL_INTERVAL_MS removed
```
