...
**Version:** 2.7 (Pre-Implementation) — §1 non-goal clarified re: client-side chart overlays vs. the "no indicator plugin marketplace" rule (see companion doc's Feature Engine kickoff, `confirmed-decisions.md` #40)
...
- No generic "add any indicator" plugin marketplace. Indicators are backend-computed and pushed as chart objects. *(This governs what can feed the algorithmic decision pipeline — Strategy/Decision Engine only ever read the one canonical Feature Engine output. It does not prohibit client-side, view-only chart-reading overlays like moving averages that a human draws on their own chart and that never reach the Event Bus; see `confirmed-decisions.md` #40.)*
...
**Chart-reading overlays (moving averages, volume averages) are client-side and view-only — a distinct category from Feature Engine output, not a second one.** The Chart widget draws two kinds of lines that look similar but come from opposite directions: backend-pushed `ChartObject`s (§4.10 — server is the source of truth, e.g. an AI-placed PDH marker) and frontend-computed indicator overlays (`frontend/src/utils/indicators.ts` — SMA today, EMA9/EMA20 already existed) that a person adds to their own chart purely to read price action, the same way they'd toggle an indicator on in any charting app. These never publish to the Event Bus, are never read by any Strategy, and have no `feature_snapshots` row — §1's "no generic indicator plugin marketplace" non-goal is about keeping *algorithmic*-decision inputs on the one canonical Feature Engine path, not about restricting what a human can overlay on a chart they're looking at themselves. `SubWindowConfig.priceIndicators` (an instance list — period/color/thickness per instance, so a chart can hold several SMAs of different periods at once) is the general shape a future overlay kind (Bollinger Bands, a VWAP band, ...) should extend, rather than each kind inventing its own config shape. See `../decisions/confirmed-decisions.md` #40.
