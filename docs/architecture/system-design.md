...
**As-built note (temp id `market-state-changed-websocket-channel`):** `MarketStateChanged` reaches the frontend the same way `ContextChanged` does (decision #126) — no new publish, only a routing entry. Cross-component flow:
...
```
 ...
 EventBus (§4.4) → WebSocket Gateway (§4.12) → "intelligence.market-state"
   ...
   │                                              temp id market-state-
   │                                              changed-websocket-channel)
   ▼
