...
| `ContextChanged` | `providers: {calendar: {...}}` — one key per active provider (§5 of the companion doc). `gap`/`levels`/`volatility_regime` never existed as real providers — decision #90 cut them before either was built. `fundamentals`/`news` join once M0's Finnhub spike results unblock `FundamentalsProvider`/`NewsFlagProvider` (see `M0-SPIKE-NOTES.md`). Confirmed decision #92 — built: `app/context_engine/engine.py`, `app/context_engine/providers/calendar.py`. |
