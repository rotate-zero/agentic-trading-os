...
**Design pass (DRAFT, nothing decided):** [`execution-engine-design.md`](./execution-engine-design.md) records where this section and the as-built code diverge — no payload model or lane decision yet for `PositionClosed`, an in-flight-order race between two signals on one symbol, and two documented owners of open positions (this section and §4.8's Position Monitor row) — as open forks EX-6 and EX-14.
...
**Design pass (DRAFT, nothing decided):** [`execution-engine-design.md`](./execution-engine-design.md) records where this section and the as-built code diverge — the `BrokerAdapter` contract has no way to report a fill, `IBKRAdapter` connects read-only with stub order methods, and the critical lane's serial consumer must never wait on a venue — plus the first vertical slice and its open forks (EX-1…EX-14).
