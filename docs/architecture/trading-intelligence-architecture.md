...
**Internal design locked; ORB, Gap, and Volume Spike (three of the seven planned strategies) are now built (decisions #99, #104, #105).** The four-stage `evaluate()` anatomy (GATE/MATCH/SCORE/PROPOSE), the Gate's two layers (per-strategy trigger + declarative environmental conditions), immutable/versioned `StrategyConfig` (family → configuration, never edited in place), and an ACT/WAIT/ABANDON entry-timing model (a strategy may act immediately or deliberately defer, never forced to wait for a candle close unless its own hypothesis specifically needs one) are locked in `strategy-engine-design.md` (decisions #87–88). `strategy_engine/` now exists: `base_strategy.py` (the real `Strategy`/`StrategyConfig`/`Opportunity`/`ScheduleTrigger`), `orb_strategy.py` (decision #99), `gap_strategy.py` (decision #104 — opening-gap continuation, `regular_open` reconstructed algebraically from `pdc`+`gap_dollars` rather than tracked as new state), and `volume_spike_strategy.py` (decision #105 — a per-candle volume-anomaly test against a private rolling per-symbol baseline, correcting `base_strategy.py`'s own illustrative `on_event("VolumeSpike")` naming to `every_candle`, since no such event exists anywhere in the codebase) — see `strategy-engine-design.md` §14 for the full build account, including an earlier undocumented attempt (`momentum_strategy.py`/`vwap_strategy.py`, built against a `base_strategy.py` that didn't exist yet) that was found orphaned and discarded rather than built on top of. Momentum and VWAP are being rebuilt fresh, assigned to a separate session, against this now-real interface; First Pullback and Reversal remain unbuilt.
