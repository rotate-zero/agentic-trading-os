...
### 174. PostgreSQL PositionLedgerPort adapter — durable receipts and a safe committed-prefix barrier

**Authorization and baseline.** Saqib approved work on the PostgreSQL adapter for `PositionLedgerPort` after updating Git. Inspected clean `main` at `898cde95f0e3e9291ba5e531adad780961a11007`, #173's contract, the existing Session implementation, ledger schema, execution/authorizer ports, and tests. Immediately before assigning this number, fetched GitHub main again: unchanged; remote/local index and log end at #173, archives end at `134-160`. Existing decision bodies remain unchanged. This builds #173's persistence seam; it does not wire startup or broaden execution behavior.

**Implementation.** `portfolio_state/postgres.py:PostgresPositionLedger(session_factory, clock=...)` implements all four synchronous operations with fresh owned SQLAlchemy transactions. It reuses the existing fill/position mappings and shared pure accounting. Each call forces READ COMMITTED and synchronous commit, then locks trades, orders, fills, positions, cursor, and receipts in `SHARE ROW EXCLUSIVE` mode. This barrier waits for outstanding writers and excludes new mutations and competing adapters until transaction end. Reads after the barrier see every previously allocated ordinary INSERT fill or its rollback. The global barrier deliberately serializes modes; no high-throughput claim is made.

**Sequence policy and migration.** Migration `0013` changes fills to `GENERATED ALWAYS AS IDENTITY` with `CACHE 1`, advances the generator past legacy explicit IDs if necessary (never rewinds), and adds `position_fill_receipts`. Default INSERT allocation occurs while holding the fill table's writer lock; the barrier therefore cannot pass an uncommitted lower allocated ID. Cache 1 prevents a connection retaining lower future IDs. The adapter checks the identity/cache policy at runtime. Administrators must not reseed/override the generator during ingestion. PostgreSQL's [lock conflict rules](https://www.postgresql.org/docs/18/explicit-locking.html) and [sequence caching behavior](https://www.postgresql.org/docs/18/sql-createsequence.html) underpin this mechanism. Unlike #173's serialized-ingestion assumption, this implementation is tested with overlapping ordinary INSERT transactions committing in reverse sequence order.

**Atomic application and restoration.** A receipt has the fill FK, unique venue fill key, mode, explicit position FK, exact fill inputs as a JSON object (Decimal strings), ET trading day, and unrestricted numeric gross delta. New applications must match the first pending authoritative fill, current cursor, shared accounting result, and fill-day attribution. Position projection, receipt, and cursor commit together; success returns only after commit. Identical already-applied fill inputs return `applied=False` with current committed state; proposed UUIDs from losing consumers do not replace the winner. Conflicting source identity, stale cursors, skips, or inconsistent calculations raise `PositionLedgerError`. Orders/status remain execution-owned; receipt quantities supply applied progress. Restore replays receipts with explicit position IDs, checks source facts and persisted projections, and restores exact lifetime/daily profit, loss, known/unknown fees, and current position stop/target/exit-attempt. Six-place position columns do not round the restored accounting state. Identical opening timestamps no longer make receipt-backed reopenings ambiguous.

**Honest boundaries.** Approved order rows are included as reservations. An approved trade without an entry order has no durable reservation quantity in the existing schema, so restoration raises until that order exists; no quantity/reference price is invented. Empty ledgers and existing unapplied fills work. Pre-0013 applied cursors/positions without complete receipts require explicit recovery; this task does not infer/backfill their history. The unchanged Session/reconciliation path remains available, but cannot jointly own a mode with this adapter. Flagged fills, overfills, incomplete history, changed source facts, corrupt projections, and mode/venue/symbol inconsistencies fail closed. Downgrade refuses to delete applied receipts. Checkpoint reconstruction reads full history; bounded-cost snapshots and lock-contention optimization await measurement. Deadlocks/database errors fail the call and require reload/retry; no success is fabricated.

**Component and internal flows.** The canonical diagrams and transaction sequence are in [execution-engine-design.md §6.5](../architecture/execution-engine-design.md#65-portfolio-state-engine-portfolio_state); §6.8 includes the new table. Worker → adapter → locked source ledger/shared arithmetic → position + receipt + cursor commit → worker cache/publication. Inside commit: barrier → checkpoint audit → duplicate check → expected-cursor/first-fill validation → recompute → atomic writes → audit → durable commit. Failures raise before acknowledgement; lost acknowledgement reloads durable state.

**Validation.** 35 new real-PostgreSQL cases plus 161 focused regressions: **196 passed, no skips** on PostgreSQL 18.6, isolated database migrated from empty through `0013`. Tests include reverse commit ordering with commit/rollback/disconnect, simultaneous duplicate consumers, stale cursors, skipped/tampered applications, rollback after writes, acknowledgement loss after commit, exact precision through restart, all four modes/interleaved sequence gaps, multi-month long/short profit/loss/fees, ET dates, terminal orders and partial reservations, reopening identities, corrupt/incomplete checkpoints, the approval-before-order gap, migration round trip/explicit-ID preservation/destructive-downgrade refusal, and real-worker commit-before-closure/restart suppression. Migration tests initially reconfigured global logging through Alembic and broke one existing log assertion; the tests now use an isolated programmatic configuration. No production behavior/assertion was weakened. Exact command and limits are in `TESTING.md`.

**Unchanged integration gaps.** No authorizer/order ledger writers, fill publisher, startup orchestration, status expansion, governor/World View adapter, exits, OutcomeRecorder, event outbox, or immutable R basis. The no-durable-notification guarantee from #173 remains. Existing legacy state import is a separate recovery task if needed. Decision archival maintenance was already overdue at baseline and is left outside the approved adapter scope.

### 175. Authorizer and order persistence — durable approval reservations and entry handoff

**Scope and baseline.** Saqib confirmed resuming the unfinished authorizer/order persistence task on 2026-09-24. Inspected `main` at `898cde95f0e3e9291ba5e531adad780961a11007` with the existing #174 adapter and unfinished integration changes; preserved that work. Immediately before this entry, fetched GitHub main: unchanged, remote index/log through #173 and archive filenames through `134-160`; local index/log agree through the pending #174. This entry closes #174's approval-before-order persistence gap and implements #171's existing ledger Protocols. It does not enable application startup.

**Durable decisions and honest rejection audits.** `governor/postgres.py:PostgresTradeLedger` commits a `TradeDecisionRecord` and, for approvals, a `trade_reservations` row in one owned transaction before returning. Migration `0014` adds exact JSON decision inputs and the reservation's deterministic entry ID, positive integer quantity, and finite positive unrestricted-numeric reference price. Approved decisions must be simulated and retain the accepted opportunity UUID; identical retries are idempotent and conflicting approval terms fail. Rejections have a separate audit-row UUID, no accepted opportunity/reservation, no invented venue, and retain the requested mode even when missing or unknown. The schema permits those rejection labels while keeping approved labels non-null, valid, and paired with the venue. This qualifies #170's mode-stamping language for rejected attempts; no paper/live execution is enabled. Signal-time snapshot-presence flags are audit inputs, not fabricated fill-time snapshots.

**Order handoff and routing.** `execution_engine/postgres.py:PostgresOrderLedger` implements both the committed-decision read and order insert/status ports. Insert rechecks approval plus reservation inside its transaction and requires the deterministic identity, symbol, side, quantity, simulated mode/venue, and market/open terms to match. Concurrent duplicates return the stored row with `inserted=False`; only a newly committed insert can reach a venue. Execution routes the returned durable instruction and requires the actual venue ID to match the committed venue as well as supporting its mode. Candidate-venue rejection never rewrites the capital's venue label. Status updates preserve partial progress, terminal states, and reconciliation-owned `unknown`; rejection persistence failure publishes no rejection notification.

**Portfolio read-back and transactions.** Portfolio State can restore and look up an approved entry before an order exists, retaining its exact reference price. Once the order exists, its status and applied fill quantity determine remaining exposure; the retained reservation is not counted twice. The decision/order adapters share fresh READ COMMITTED, synchronous-commit transactions locking trades, orders, then reservations. The Portfolio adapter takes the same lock prefix before fills, positions, cursor, and receipts. Existing order-backed history remains readable without invented reservation backfill; a legacy approval with neither order nor reservation still fails explicitly. Internal and component flows are documented in [execution-engine-design.md §§6.2–6.5](../architecture/execution-engine-design.md#62-authorizer-stub-governor--name-deliberately-provisional).

**Validation and migration safety.** Fresh isolated database upgraded through `0014`; **254 focused tests passed, no skips**, including 58 authorizer/order PostgreSQL cases plus #174's 196 regressions. Coverage includes decision/order rollback and commit-acknowledgement loss, concurrent duplicates, approval/publication and order/venue commit boundaries, tampered terms, venue mismatch, rejection audit labels, partial-fill handoff, stale status preservation, migration round trips, and refusal to downgrade away reservations or rejection audits. Exact environment and command are in `TESTING.md`. Existing decision bodies remain unchanged.

**Remaining boundaries.** Startup, the governor Portfolio-State read adapter/cache synchronization, venue-fill persistence/publication, reconciliation/recovery orchestration, exits, outcomes, and durable notification delivery remain separate. If an order commit acknowledgement is lost, the persisted approval is retained and retry does not blindly resubmit; a later recovery path must reconcile it. An approval persisted before publication can likewise remain reserved after notification loss. These adapters do not make multi-authorizer risk checking atomic. Global locks favor correctness over throughput. Migration `0014` refuses destructive downgrade when authorization records/reservations exist; it was applied only to the isolated test database. Pre-existing decision-log archival maintenance remains outside this focused delivery.
