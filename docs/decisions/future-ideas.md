...
**Status:** resolved, not reopened. `PolygonAdapter` (`confirmed-decisions.md` #30) and `FinnhubAdapter` (`confirmed-decisions.md` #32) are both built, and the later IBKR historical Backtest Runner path adds real one-minute replay OHLCV without changing those live/chart provider roles. This entry stays on record for provider-comparison reasoning, not as an unresolved selection decision. Polygon and Finnhub aren't redundant with each other — Polygon serves historical backfill (which Finnhub's free tier can't do at all), Finnhub serves real-time streaming (which Polygon's free tier can't do at all) — see `confirmed-decisions.md` #33 for how they're coordinated.
...
## 27. Real-IBKR paper-session validation of the broker panel — deferred (temp id `ibkr-broker-panel-validation-deferred`)

**What it is:** the first empirical validation of `backend/app/api/routes/broker.py`'s five routes (`connect`/`subscribe`/`unsubscribe`/`status`/`disconnect`) and `BrokerPanel.tsx` / `useBrokerStatus.ts` against a real, already-authenticated IBKR **Paper** session — same spirit as `backend/scripts/check_premarket_data_availability.py`: real account, real findings, honestly reported. Tooling and runbook already exist: `backend/scripts/check_ibkr_broker_panel.py` and `../architecture/ibkr-broker-panel-validation.md`.

**Why deferred:** blocked, then deliberately set aside by Saqib on 2026-09-19. Two independent checks found no reachable paper session: (1) the authoring sandbox — TCP to `127.0.0.1:4002` refused, and that environment has no route to his machine; (2) a local session on Saqib's machine — no listener on port 4002, no TWS or IB Gateway process, configured endpoint `127.0.0.1:4002`, client ID `1`. Nothing was changed, no route was called, `broker.py` was not touched. Client-ID uniqueness could not be validated without an active session. The harness was exercised only against a stub backend (fake `IB`, real routers/Event Bus/WebSocket), which validates the script's own logic and guards, **not** IBKR, `IBKRAdapter`, or the UI. Consequence: nothing new is known about real IBKR behaviour; `backend/README.md`'s ❌ "actual live connection" line stands, and the seven code-reading predictions (P1–P7 in the runbook) remain hypotheses, not findings.

**Open items surfaced while building the harness, not decided (awaiting Saqib):**
- P5 — `IBKRAdapter.connect` passes no `fetchFields`, so ib_async's default (`StartupFetchALL`: positions, open/completed orders, account updates, executions) syncs paper account snapshot data into memory on every connect. Whether to pass an explicit `fetchFields` for a data-only adapter is a design fork.
- P1 — `GET /broker/status` reports whichever provider is streaming, not IBKR specifically; with Finnhub auto-connected the panel would show "● Connected" and disable its Connect button. Reproduced at the route level with a stand-in provider only. Belongs to `broker.py`'s owner.

**Trigger to revisit:** a paper IB Gateway (port 4002) or TWS (7497) is running and logged in on Saqib's machine (IB Key approved, API socket enabled, `127.0.0.1` trusted), **and** he chooses to run it, with a client ID no other active process uses. Don't revisit on "close enough": no listener on the port means blocked, not "nearly ready". Separately, treat the IBKR adapter path and the broker panel as **unverified live** in any Phase 4 (ConnectionManager / auto-reconnect) or Phase 5/6 (execution) planning until this has run.

**Where it would plug in:** follow the runbook in `../architecture/ibkr-broker-panel-validation.md` (start with its "Deferral record → Resume checklist"). Results ship as their own delivery (temp id `ibkr-broker-panel-validation-results`) that flips the README ❌ line and carries the decision-log entry, numbered only at merge after re-checking `main` and both decision logs. Any finding that real behaviour differs from the UI/docs about `broker.py` is documented precisely and handed back, never patched from that track.
